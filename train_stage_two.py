...
from utils import get_config, flatten_omega_conf, AverageMeter, path_to_llm_name, _count_params, \
    collect_lora_targets_full, collect_modules_to_save_for_full_ft, mask_grad_last_k_rows
...
def main():
    ...
    if config.model.showo.load_from_showo:
        ...
        if not bool(config.model.showo.add_time_embeds):
            raise ValueError(
                "model requires model.showo.add_time_embeds=True"
            )
        model.register_to_config(add_time_embeds=True)
        if not bool(model.config.add_time_embeds):
            raise RuntimeError(
                "add_time_embeds was not registered in model.config"
            )
        # config the showo-2 weight
        load_path = config.model.showo.pretrained_model_path
        if not os.path.isfile(load_path):
            raise FileNotFoundError(
                f"Show-o2 base checkpoint not found: {load_path}"
            )
    ...
    modules_to_save = collect_modules_to_save_for_full_ft(model)  # lm_head + cond_proj
    ...
    if config.model.showo.lora_path is not None:
        ...
        model = PeftModel.from_pretrained(
            model,
            config.model.showo.lora_path,
            is_trainable=True,
        )

        # Existing adapters use their own adapter_config.json.
        loaded_lora_cfg = model.peft_config["default"]

        if int(loaded_lora_cfg.r) != int(lora_cfg.rank):
            raise ValueError(
                f"LoRA rank mismatch: adapter={loaded_lora_cfg.r}, "
                f"yaml={lora_cfg.rank}"
            )

        if int(loaded_lora_cfg.lora_alpha) != int(lora_cfg.alpha):
            raise ValueError(
                f"LoRA alpha mismatch: adapter={loaded_lora_cfg.lora_alpha}, "
                f"yaml={lora_cfg.alpha}"
            )
    else:
    ...
    optimizer_grouped_parameters = [
        ...
        {
            "params": [p for n, p in model.named_parameters() if ((
                                                                              'cond_proj' in n or 'lm_head' in n or 'segmentor' in n or 'fusion_proj' in n or '.spe.' in f'.{n}.') and p.requires_grad)],
            "weight_decay": optimizer_config.weight_decay,
    ...
    optimized_parameter_ids = set()
    for group in optimizer_grouped_parameters:
        unique_parameters = []
        for parameter in group["params"]:
            if id(parameter) not in optimized_parameter_ids:
                unique_parameters.append(parameter)
                optimized_parameter_ids.add(id(parameter))
        group["params"] = unique_parameters
    optimizer_grouped_parameters = [
    ...
    missing_trainable_parameters = [
        ...
        if parameter.requires_grad and id(parameter) not in optimized_parameter_ids
    ]
    if missing_trainable_parameters:
        raise RuntimeError(
            "Trainable parameters missing from optimizer groups: "
            + ", ".join(missing_trainable_parameters)
        )
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch in mixed_loader:
            with accelerator.accumulate(model):
                ...
                raw_text_labels = batch["text_labels"]
                if isinstance(raw_text_labels, torch.Tensor):
                    text_labels = raw_text_labels.to(accelerator.device)
                else:
                    text_labels = None
                ...
                image_latents, t, image_labels, image_masks, alpha_t = prepare_latents_and_labels(
                    pixel_values,
                    batch["data_type"],
                    image_masks,
                )
                ...
                logits, loss, seg_loss_summary = model(
                ...
                seg_loss, focal_ce_loss, dice_loss = seg_loss_summary
