    ...
    def sample(self, videos, deterministic=False, return_features=False):
        with amp.autocast(dtype=self.dtype):
            if return_features:
                ...
                return self.model.sample(videos, self.scale, deterministic=deterministic).float()
        ...
        with amp.autocast(dtype=self.dtype):
            if return_features:
                ...
                for i in range(videos.shape[0]):
                    out, feats = self.model.sample(videos[i][None], self.scale, deterministic=deterministic,
                                                   return_features=return_features)
                    outputs.append(out)
                ...
                outputs = []
                for i in range(videos.shape[0]):
                    out = self.model.sample(videos[i][None], self.scale, deterministic=deterministic).float()
                    outputs.append(out)
                outputs = torch.cat(outputs, dim=0)
