...
class TokensToVolume(nn.Module):
    def __init__(self, spatial_size: Tuple[int, int, int] = (192, 192, 40),
        ...
        if share_in_proj:
            ...
            self.in_proj_low = nn.Conv3d(base_ch, C0, kernel_size=1, bias=False)
            self.in_proj_middle = nn.Conv3d(base_ch, C0, kernel_size=1, bias=False)
            self.in_proj_high = nn.Conv3d(base_ch, C0, kernel_size=1, bias=False)
