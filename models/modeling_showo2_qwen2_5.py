...
from .condLoRA import ConcatFusion_M
...
class SpatialAPE3D_HWD(nn.Module):
    """
    3D Absolute Positional Encoding (Spatial).

    Args:
        dim: Feature dimension.
        spatial_size: Tuple representing (H, W, D).
        comp: Tuple representing compression factors (cD, cH, cW).
        num_freqs: Number of frequency bands for Fourier features.
        base: Base multiplier for frequency scaling.
        learnable_proj: Whether the projection layer parameters are updated during training.
        pos_scale: Scaling factor applied to the final positional embeddings.
    """
    ...
    def __init__(
        ...
        if H % cH != 0 or W % cW != 0 or D % cD != 0:
            raise ValueError(f"spatial_size {spatial_size} must be perfectly divisible by comp {comp}.")

        self.Hg, self.Wg, self.Dg = H // cH, W // cW, D // cD
        self.N = self.Dg * self.Hg * self.Wg

        # Match the latent/image-token flattening order: (D, H, W). Use a
        # half-open interval so the first and last positions do not alias.
        z = torch.arange(self.Dg, dtype=torch.float32) / self.Dg
        y = torch.arange(self.Hg, dtype=torch.float32) / self.Hg
        x = torch.arange(self.Wg, dtype=torch.float32) / self.Wg
        zz, yy, xx = torch.meshgrid(z, y, x, indexing='ij')

        # Multi-frequency Fourier features: shape (N, 6F)
        freqs = self.base ** torch.arange(self.num_freqs)

        def get_fourier_embed(t: torch.Tensor) -> torch.Tensor:
            t = 2 * torch.pi * (t[..., None] * freqs)
            return torch.cat([torch.sin(t), torch.cos(t)], dim=-1)

        fy = get_fourier_embed(yy)
        fx = get_fourier_embed(xx)
        fz = get_fourier_embed(zz)

        feats = torch.cat([fy, fx, fz], dim=-1).reshape(self.N, -1)
        ...
        # Zero-initialize to prevent early perturbation
        with torch.no_grad():
            nn.init.zeros_(self.proj.weight)

    def forward(self, x_like: torch.Tensor) -> torch.Tensor:
        # Project features dynamically to allow gradient updates
        feats = self.feats.to(device=x_like.device, dtype=x_like.dtype)
        pos = self.proj(feats) * self.pos_scale
...
class Showo2Qwen2_5(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        super().__init__()
        if not add_time_embeds:
            raise ValueError(
                "Model requires add_time_embeds=True"
            )
    ...
    def reset_vocbulary(self, tokenizer=None):
        ...
        class_tokens = [ "<CRT>",  "<RT>", "<TMZ>", "<SURGERY>", "<AM>", ]
        class_token_ids = [
            tokenizer.convert_tokens_to_ids(token)
            for token in class_tokens]
        expected_ids = list(
            range(len(tokenizer) - len(class_tokens), len(tokenizer)) )

        if class_token_ids != expected_ids:
            raise RuntimeError(
                "Treatment class tokens must occupy the final five "
                f"vocabulary positions: got {class_token_ids}, "
                f"expected {expected_ids}")

        self.register_to_config(llm_vocab_size=len(tokenizer))
    ...
    def forward(
        ...
        if image_latents is None:
            ...
            # Conditional and spatial features are aligned to (D, H, W) token
            # order and must be added once, outside the modality-position loop.
            if image_latents_cond is not None:
                cond_embeds, _ = self.cond_proj(image_latents_cond.to(dtype))
                if cond_embeds.shape != image_embeds.shape:
                    raise ValueError(
                        "conditional/image embedding shape mismatch: "
                        f"{cond_embeds.shape} vs {image_embeds.shape}"
                    )
                image_embeds = image_embeds + cond_embeds

            image_embeds = image_embeds + self.spe(image_embeds)
            ...
            logits, last_hidden_states = outputs["logits"], outputs["hidden_states"][-1]
            if mode != "train" and image_latents_cond is None:
                return logits, None

            seg_predicted_mask = None
            ...
            if seg_masks is not None:
                ...
                input_seg0, input_seg1, input_seg2 = [], [], []
                for i, modality_batch in enumerate(modality_positions):
                    for j, (offset, length) in enumerate(modality_batch):
                        input_seg0.append(
                            hidden_states0[i, offset + 1:offset + length].unsqueeze(0)
                        )
                        input_seg1.append(
                            hidden_states1[i, offset + 1:offset + length].unsqueeze(0)
                        )
                        input_seg2.append(
                            hidden_states2[i, offset + 1:offset + length].unsqueeze(0)
                        )

                seg_predicted_mask = self.segmentor(
                    torch.cat(input_seg0),
                    torch.cat(input_seg1),
                    torch.cat(input_seg2),
                )
                seg_loss, focal_ce_loss, dice_loss = seg_prediction(
                    seg_predicted_mask,
                    seg_masks,
                    item_alpha=alpha_t,
                )
