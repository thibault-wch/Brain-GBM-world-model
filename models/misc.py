...
def seg_prediction(
    logits: torch.Tensor,           # (B,4,D,H,W)
    labels_1ch: torch.Tensor,       # (B,1,D,H,W) in {0,1,2,3}
    item_alpha = None,
    gamma: float =1.0,
    alpha = None,
    smooth: float = 1.0,
    ...
    assert C == 4, f"expect 5 classes, got {C}"
    ...
    if labels.dtype.is_floating_point:
        labels = labels.round()
    labels = labels.clamp(0, NUM_CLASSES - 1).long()

    # ---------- Focal Cross-Entropy (include bg=0) ----------
    # log_probs: (B,4,D,H,W)
    log_probs = F.log_softmax(logits.float(), dim=1)
    # 取 gt  log p_t: (B,D,H,W)
    log_pt = log_probs.gather(1, labels.unsqueeze(1)).squeeze(1)
    pt = log_pt.exp()

    alpha_t = 1.0
    focal_ce_per_item = -(alpha_t * (1.0 - pt).clamp_min(EPS).pow(gamma) * log_pt).mean(dim=(1,2,3))         
    ...
    inter = (probs_fg * one_hot_fg).sum(dim=(2,3,4))                
    denom = (probs_fg + one_hot_fg).sum(dim=(2,3,4))                
    dice_loss = 1.0 - (2.0 * inter + smooth) / (denom + smooth)
    dice_loss_per_item = dice_loss.mean(dim=1)

    # ---------- per-item  ----------
    total_per_item = focal_ce_per_item + dice_loss_per_item       # (B,)

    if item_alpha is not None:
        # print('enter item alpha',item_alpha)
        w = torch.as_tensor(item_alpha, device=device, dtype=torch.float32)
        if w.ndim == 0:
            w = w.expand(B)
        else:
            w = w.view(-1)
            if w.numel() < B:
                w = torch.cat([w, w.new_zeros(B - w.numel())])
            elif w.numel() > B:
                w = w[:B]
        w = torch.nan_to_num(w, nan=0.0).clamp_min(0.0)


        total = (w * total_per_item).mean()
        ce    = (w * focal_ce_per_item).mean()
        dice  = (w * dice_loss_per_item).mean()
    else:
        total = total_per_item.mean()
        ce    = focal_ce_per_item.mean()
        dice  = dice_loss_per_item.mean()
...
def next_token_prediction(logits, labels, vocab_szie):
    # print(f'vocab information: {logits.shape},{vocab_szie}')
    return F.cross_entropy(logits[:, :-1].contiguous().view(-1, 5), 
...
# ============================================
# 5类 → 单 token 的 special tokens
# ============================================
SPECIAL_CLASS_TOKENS = ["<CRT>", "<RT>", "<TMZ>", "<SURGERY>", "<AM>"]

# === Chwang new added: class-aware tokenization & special-token init (clean) ===
import re
import torch
import torch.nn as nn

# Canonical map name (fallback to existing constants if present)
CLASS2TOKEN_MAP = {
            "crt_id": "<CRT>",
            "rt_id": "<RT>",
            "tmz_id": "<TMZ>",
...
DEFAULT_ALIAS = {
    "<CRT>":      ["chemoradiotherapy", "chemo radiotherapy", "chemo-radiation", "CRT"],
    "<RT>":       ["radiotherapy", "radiation", "RT"],
    "<TMZ>":      ["temozolomide", "tmz", "temodar"],
    "<SURGERY>":  ["surgery", "resection", "operation"],
    "<AM>":       ["active monitoring", "watchful waiting", "observation"],
}

def _ids_for_phrase(tokenizer, phrase: str):
    return tokenizer(phrase, add_special_tokens=False)["input_ids"]

def _mean_emb(emb_table, ids):
    if len(ids) == 1:
        return emb_table[ids[0]].clone()
    return emb_table[ids, :].mean(dim=0)

@torch.no_grad()
def init_special_token_embeddings(
    model,
    tokenizer,
    class2token_map=CLASS2TOKEN_MAP,
    alias_map=DEFAULT_ALIAS,
):
    special_tokens = {"additional_special_tokens": []}
    class_tokens = list(dict.fromkeys(class2token_map.values()))
    special_tokens["additional_special_tokens"].extend(class_tokens)
    try:
        if "SPECIAL_CLASS_TOKENS" in globals():
            for t in globals()["SPECIAL_CLASS_TOKENS"]:
                if t not in special_tokens["additional_special_tokens"]:
                    special_tokens["additional_special_tokens"].append(t)
    except Exception:
        pass
    ...
    try:
        ...
        out_emb, has_output = None, False
    for tok in special_tokens["additional_special_tokens"]:
        sid = tokenizer.convert_tokens_to_ids(tok)
        if sid is None or sid < 0:
            continue
        best_vec = None
        for phrase in alias_map.get(tok, []):
            ids = _ids_for_phrase(tokenizer, phrase)
            if not ids:
                continue
            vec = _mean_emb(emb, ids)
            if len(ids) == 1:
                best_vec = vec
                break
            if best_vec is None:
                best_vec = vec
        if best_vec is None:
            best_vec = torch.empty_like(emb[0])
            nn.init.normal_(best_vec, mean=0.0, std=emb.shape[1] ** -0.5)
        emb[sid].copy_(best_vec)
        if has_output and out_emb is not None and out_emb.data_ptr() != emb.data_ptr():
            out_emb[sid].copy_(best_vec)
    return tokenizer
