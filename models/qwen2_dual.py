...
# from misc import seg_prediction
...
_CHECKPOINT_FOR_DOC = "Qwen/Qwen2-1.5B"
...
# Copied from transformers.models.llama.modeling_llama.rotate_half HxW. 1536 768 1s
def rotate_half(x):
...
class Qwen2DecoderLayerDual(GradientCheckpointingLayer):
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: Qwen2Config):
        ...
        basic_layers=config.share_layer_num[0]
        self.layers = nn.ModuleList(
            [Qwen2DecoderLayer(config, layer_idx) for layer_idx in range(basic_layers)]
        )
        self.dual_layers=nn.ModuleList(
            [Qwen2DecoderLayerDual(config, layer_idx+basic_layers) for layer_idx in range(config.num_hidden_layers-basic_layers)]
        )
    ...
    def forward(
        ...
        # middle hidden latents
        # mid_hidden_states= hidden_states
        for decoder_layer in self.dual_layers:
            if output_hidden_states:
                all_hidden_states += (hidden_states,)
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        # # logits preprocess for metrics
        
        def preprocess_logits_for_metrics_generative_for_classification(logits):
            logits = logits[0] if isinstance(logits, tuple) else logits
            # print(logits.shape)
            logits =logits[:,:,-5:]
            return logits
            # return preprocess_logits_for_metrics_generative_for_classification
        logits=preprocess_logits_for_metrics_generative_for_classification(logits=logits)
