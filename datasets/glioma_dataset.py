...
class MedicalPairImageTextDataset(Dataset):
    ...
    def __init__(
        ...
        if self.is_captioning:
            # MMU training additionally contains one class token.
            reserved_tokens = 5 if self.mode == "train" else 4
            self.max_text_len = (
                    max_seq_len
                    - num_image_tokens
                    - reserved_tokens
            )
        elif len(self.system_tokens[0]) == 0:
        ...
        self.img_t = mtransforms.Compose([
            ...
            mtransforms.ScaleIntensityRangePercentilesd(
                ...
                lower=0, upper=99.9, b_min=-1.0, b_max=1.0, clip=True, relative=False),
