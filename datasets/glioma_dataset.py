...
def _dedup_test_caption_by_tid1(anno: List[Dict[str, str]]) -> List[Dict[str, str]]:
    """
    在 test + captioning 情况下：
    如果 t1_time_id（tid1）重复，只保留第一次出现的那条记录。
    去重键使用：site + normalized_tid1 + patient_id（更安全，避免不同病人同 tid1 被误杀）
    如果你“只想按 tid1 全局去重”，把 key 里的 pid/site 去掉即可。
    """
    seen = set()
    kept = []
    for r in anno:
        ...
        # ✅ 推荐：同一个站点/病人内 tid1 去重（最合理）
        key = (site, pid, tid1)

        # 如果你坚持“只按 tid1 全局去重”，用这一行：
        # key = tid1
...
REQUIRED_COLS  = {"patient_id", "t1_time_id", "t2_time_id", "template_text_en", "conducted_treatment"}
# 若希望稳定列顺序，可用以下列表（不影响逻辑）
# REQUIRED_ORDER = ["patient_id", "t1_time_id", "t2_time_id", "template_text_en", "conducted_treatment"]

def _read_and_validate_pair_csvs(csv_paths: List[str], external_csv_paths: List[str], mode: str = "train") -> List[Dict[str, str]]:
    """
    读取多份 CSV：
    - 仅保留 REQUIRED_COLS + "__site__" 列
    - 根据 test 列表与 mode(train/test) 过滤
    - 所有值转为字符串并去除首尾空格
    - "__site__" 取自文件名（不含扩展名，转大写）
    """
    assert mode in {"train", "test","external"}, "mode 必须是 'train' 或 'test'"
    ...
    if mode =='train' or mode =='test':
        ...
        for pth in present:
            ...
            if missing:
                raise ValueError(f"{pth} 缺少必需列: {sorted(missing)}")

            # 仅取所需列，并做值清洗
            cols = [c for c in REQUIRED_COLS  if c in df.columns]  # 用稳定顺序
            df = df[cols].applymap(lambda x: x.strip() if isinstance(x, str) else ("" if pd.isna(x) else str(x).strip()))
        ...
        if not present:
            return []
        for pth in present:
            base = os.path.splitext(os.path.basename(pth))[0]  # 比如 "RHUH_v2" 或 "RHUH"
...
def format_sequence_gen_qwen2_5(text_tokens, system_tokens, bos_id, eos_id, boi_id, eoi_id, pad_id, img_pad_id,
    ...
    if system_token_len == 0:
        ...
        text_labels = [bos_id] + [-100] * len(system_tokens[0] + system_tokens[1] + text_tokens) + [eos_id] +                       [-100] * len(system_tokens[2]) +                       [boi_id] + [-100] * num_image_tokens + [eoi_id] + [eos_id]
        text_tokens = [bos_id] + system_tokens[0] + system_tokens[1] + text_tokens + [eos_id] + system_tokens[2] +                       [boi_id] + [img_pad_id] * num_image_tokens + [eoi_id] + [eos_id]
...
def format_sequence_und_masked(prompt_tokens, answer_tokens, bos_id, eos_id, boi_id, eoi_id, pad_id, img_pad_id,
    ...
    Layout:
      [bos, boi, <img_pad>*N, eoi, prompt_tokens + answer_tokens, eos, pad...]
    Labels:
      [bos=-100, boi=-100, <img_pad> part=-100, eoi=-100, -100*len(prompt) + answer_tokens, -100, pad...]
    """
    modality_positions = torch.tensor([[1 + 1, num_image_tokens]])
    # print('answer_tokens',answer_tokens)
    answer_tokens[0]-=151669
    # print('answer_tokens',answer_tokens)
    # Build text tokens
    text_tokens = [bos_id] + [boi_id] + [img_pad_id] * num_image_tokens + [eoi_id] + prompt_tokens + answer_tokens + [eos_id]

    # Build labels: ignore everything except answer tokens
    text_labels = (
        [-100] +                    # bos
        [-100] +                    # boi
        [-100] * num_image_tokens + # image tokens
        [-100] +                    # eoi
        [-100] * len(prompt_tokens) +  # mask prompt
        answer_tokens +             # supervise answer
        [-100]                      # eos
    )
...
def format_sequence_und_infer(prompt_tokens, answer_tokens,
                               bos_id, eos_id, 
                               boi_id, eoi_id, 
                               pad_id, img_pad_id,
                               num_image_tokens, max_seq_len):
...
# ---- Conducted treatment -> 5 special tokens ----
CLASS2TOKEN_MAP = {
    "CRT": "<CRT>",
    "RT": "<RT>",
    "TMZ": "<TMZ>",
...
# case-insensitive patterns to canonical class
import re as _re
_CANON_PATTERNS = [
...
def replace_with_special_tokens(text: str) -> str:
    """
    将输入文本中匹配到的治疗术语替换为对应的特殊token，如 'CRT' -> '<CRT>'。
    其他未匹配部分保持不变。
    """
    ...
    for pattern, cls in _CANON_PATTERNS:
        ...
        # 用 lambda 保持原文大小写结构被替换成固定 token
        result = pattern.sub(lambda m: f"{token}", result)
    return result

class MedicalPairImageTextDataset(Dataset):
    """
    两种模式：
      - mmu (is_captioning=True): images=t1_image，texts=conducted_treatment_en（建议单token），监督文本；
      - t2i (is_captioning=False): images_cond=t1_image，images=t2_image，texts=template_text_en，文本不监督。
    目录结构：root/patient_id/time_id/{flair.nii.gz,t1ce.nii.gz,t2.nii.gz,seg_mask.nii.gz}
    """
    def __init__(
        self,
        root: str,
        ...
        img_keys = ["flair1", "t1ce1", "t2w1","flair2", "t1ce2", "t2w2"] + (["seg1","seg2"] if self.use_seg_mask else [])
        print(img_keys)
        # MONAI transforms
        self.img_t = mtransforms.Compose([
            mtransforms.LoadImaged(keys=img_keys,image_only=True, ensure_channel_first=True), 
            mtransforms.EnsureChannelFirstd(keys=img_keys),  
            mtransforms.SqueezeDimd(keys=img_keys), 
            mtransforms.Spacingd(keys=["flair1", "t1ce1", "t2w1","flair2", "t1ce2", "t2w2"], pixdim=(1.0,1.0,1.0), mode=("bilinear",)*6),
            mtransforms.Spacingd(keys=["seg1","seg2"], pixdim=(1.0,1.0,1.0), mode=("nearest",)*2, allow_missing_keys=True),
            # mtransforms.EnsureChannelFirstd(keys=img_keys),   
            mtransforms.EnsureTyped(keys=img_keys), 
            mtransforms.ScaleIntensityRangePercentilesd(
                keys=["flair1", "t1ce1", "t2w1","flair2", "t1ce2", "t2w2"],
                lower=3, upper=97, b_min=-1.0, b_max=1.0, clip=True, relative=False),
            mtransforms.SpatialCropd(
                keys=img_keys,
                roi_center=(120,120,78),roi_size=(192, 192,140)),
            mtransforms.Resized(keys=["flair1", "t1ce1", "t2w1","flair2", "t1ce2", "t2w2"],spatial_size=(spatial_size[0],spatial_size[1],spatial_size[2]), allow_missing_keys=True),
            mtransforms.Resized(keys=["seg1","seg2"],spatial_size=(spatial_size[0],spatial_size[1],spatial_size[2]), mode='nearest', allow_missing_keys=True),
        ])
        def _zoom_like_crop(keys, use_seg):
            modes = (
                ("bilinear","bilinear","bilinear","bilinear","bilinear","bilinear","nearest","nearest")
                if use_seg else ("bilinear",)*6
            )
            try:
                return mtransforms.RandZoomd(
                    keys=keys, prob=1.0,
                    min_zoom=1.0, max_zoom=1.5,   
                    mode=modes, keep_size=True,  
                )
            except TypeError:
                return mtransforms.RandZoomd(
                    keys=keys, prob=1.0,
        ...
        self.random_transform=mtransforms.OneOf(
                ...
                mtransforms.RandAffined(
                    ...
                    mode=("bilinear","bilinear","bilinear","bilinear","bilinear","bilinear",
                        "nearest","nearest") if self.use_seg_mask else ("bilinear",)*6,
                ),
        ...
        # Hard-coded read & validate CSVs
        self.anno: List[Dict[str, str]] = _read_and_validate_pair_csvs(DEFAULT_CSVS,EXTERNAL_CSVs,self.mode)
        # if self.mode == "test" and self.is_captioning:
        #     before = len(self.anno)
        #     self.anno = _dedup_test_caption_by_tid1(self.anno)
        #     after = len(self.anno)
        #     print(f"[Dedup] test+caption: {before} -> {after} (drop {before-after} by duplicated tid1)")
        if self.mode =="external" and self.is_captioning:
            self.anno=self.anno[20:]
        print('dataset length:', len(self.anno))

    def __len__(self) -> int:
        return len(self.anno)

    # def _load_img3d(self, p: str) -> torch.Tensor:
    #     x = self.img_t(p)              # (C,H,W,D)
    #     if x.ndim == 4:
    #         x = x.permute(0, 3, 1, 2)  # (C,D,H,W)
    #     return x.contiguous()

    # def _load_seg3d(self, p: str) -> torch.Tensor:
    #     x = self.seg_t(p)              # (C,H,W,D)
    #     if x.ndim == 4:
    #         x = x.permute(0, 3, 1, 2)  # (C,D,H,W)
    #     return x.contiguous()
    ...
    def _remap_seg_by_site(self, seg: torch.Tensor, site: str) -> torch.Tensor:
        ...
        """
        site = (site or "").upper() 
        seg_new = seg.clone()               # 不修改原 tensor 的引用
        if site == "LUMIERE":
            m1 = seg == 1
            m2 = seg == 2
            seg_new[m1] = 2
    ...
    def _stack_modalities(self, flair1: str, t1ce1: str, t2w1: str,
                          flair2: str, t1ce2: str, t2w2: str,seg1:str = None,seg2:str = None,
                          mode='train') -> torch.Tensor:
        if seg1 == None and seg2==None:
            if mode == 'train': 
                # self.random_transform 
                dict_img = self.random_transform (self.img_t({
                    'flair1':flair1,
                    't1ce1':t1ce1,
                    't2w1':t2w1,
                    'flair2':flair2,
            ...
            return torch.cat([
                ...
                dict_img['t2w1'].permute(0,3,1,2)], dim=0),torch.cat([
                dict_img['flair2'].permute(0,3,1,2),
                dict_img['t1ce2'].permute(0,3,1,2),
                dict_img['t2w2'].permute(0,3,1,2)], dim=0),
            ...
            # print(dict_img['flair1'].shape)
            return torch.cat([
                dict_img['flair1'].permute(0,3,1,2),
                dict_img['t1ce1'].permute(0,3,1,2),
                dict_img['t2w1'].permute(0,3,1,2)], dim=0), dict_img['seg1'].permute(0,3,1,2), torch.cat([
                dict_img['flair2'].permute(0,3,1,2),
                dict_img['t1ce2'].permute(0,3,1,2),
                dict_img['t2w2'].permute(0,3,1,2)], dim=0),dict_img['seg2'].permute(0,3,1,2),
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        try:
            ...
            # Normalize time IDs per site rules
            ...
            if self.use_seg_mask:
                need += [seg1, seg2]
                t1_img,t1_seg,t2_img,t2_seg = self._stack_modalities(flair1, t1ce1, t2w1,flair2, t1ce2, t2w2,seg1,seg2,self.mode)
            else:
                t1_img,t2_img = self._stack_modalities(flair1, t1ce1, t2w1,flair2, t1ce2, t2w2,None,None,self.mode)
            if self.strict_files:
                for pth in need:
                    if not os.path.isfile(pth):
                        raise FileNotFoundError(f"lost file : {pth}")  

            if self.is_captioning:  # mmu
                full_tpl = r.get("template_text_en", "") or ""
                # 用正则匹配，无论"conducted treatment"是否大写
                ...
                # print(prompt_text,len(prompt_text),prompt_tokens,len(prompt_tokens))
                ...
                # print(special,len(special),answer_tokens,len(answer_tokens))
                if self.mode =='train':
                ...
                sample = {
                    ...
                    "texts": (prompt_text + " " + special).strip() if self.mode=='train' else [(prompt_text + " ").strip(),special],
                    "data_type": "mmu",
                    "file_id":tid1+'_'+tid2
                }
                if self.use_seg_mask:
            ...
            print(f'pid = {pid}, site={site} error in data loading: {e}, try next idx')
    ...
    def collate_fn(self, batch):
        ...
        if len(batch) == 0:
            # 你也可以 return None，然后上层跳过；这里返回空 dict 更安全
            return {}
        ...
        # 3) 仅当“所有元素都是 Tensor 且形状兼容”才 stack；否则保留为 list
        for k, vlist in list(batched.items()):
            # 跳过显然是文本/元数据的键：可选
            # if k in ("texts", "data_type", "site", "pid"):
            #     continue
            ...
            if all(isinstance(x, torch.Tensor) for x in vlist):
                try:
                    ...
                    # 形状不一致就别 stack，保留 list 以免崩溃
                    batched[k] = vlist
            else:
                # 不是 Tensor（str / int / dict...） -> 统一保留 list
                # 注意：如果你希望把标量数字也转 tensor，可以加一条分支做 torch.tensor(vlist)
                pass
...
def create_medical_dataloader(
    ...
    dataloader = DataLoader(ds, batch_size=batch_size,
                            sampler=sampler, collate_fn=ds.collate_fn,
                            shuffle=shuffle, num_workers=num_workers,
                            drop_last=drop_last)
    return dataloader

    # return DataLoader(ds, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, collate_fn=ds.collate_fn)
