...
class MixedDataLoader:
    def __init__(self,
        ...
        self.loader_epochs = [-1] * len(self.loader_list)
        self.iter_list = [None] * len(self.loader_list)

    def _restart_loader(self, idx):
        self.loader_epochs[idx] += 1

        sampler = getattr(self.loader_list[idx], "sampler", None)
        if hasattr(sampler, "set_epoch"):
            sampler.set_epoch(self.loader_epochs[idx])

        self.iter_list[idx] = iter(self.loader_list[idx])

    def __iter__(self):
        for idx in range(len(self.loader_list)):
            self._restart_loader(idx)
    ...
    def _sequential_max_size_cycle(self):
        ...
        while loaders_used < self.n_iters_per_sequential_iter:
            try:
                ...
                self._restart_loader(self.current_loader_idx)
                batch = next(self.iter_list[self.current_loader_idx])
                batched.append(batch)
