# Reported benchmark metrics are computed after full-volume padding restores
# both predictions and references to the original spatial extent.
...
if __name__ == '__main__':
    ...
    if not bool(config.model.showo.add_time_embeds):
        raise ValueError("model requires model.showo.add_time_embeds=True")
    model.register_to_config(add_time_embeds=True)
    if not bool(model.config.add_time_embeds):
        raise RuntimeError("add_time_embeds was not registered in model.config")
    ...
    state_dict = convert_qwen2_to_qwen2_dual(
        torch.load(load_path, map_location="cpu", weights_only=True, mmap=True,),
